...
const ImportModalUpdate = ({ opened, close, setValue }: Props) => {
  ...
  const handleImport = async () => {
    if (!importBillId || !importType) {
      toast.error("Please select both type and bill");
      return;
    }

    try {
      const response = await fetch(`/api/bills/${importType}/${importBillId}`, {
        method: "GET",
        headers: {
    ...
    >
      <div className="space-y-6 p-1 my-2">
        <Select
          ...
          value={importType}
          onChange={(val) =>
            setImportType(val as "invoices" | "proforma-invoices")
          }
          data={[
            { value: "invoices", label: "Invoice" },
            { value: "proforma-invoices", label: "Proforma Invoice" },
          ]}
          styles={{
            input: {
              borderColor: "#cbd5e1",
        ...
        <Select
          ...
          disabled={!importType || bills.length === 0 || isLoading}
          styles={{
            input: {
              borderColor: "#cbd5e1",
              borderRadius: "8px",
        ...
        {bills.length === 0 && importType && !isLoading && (
          <div className="text-center text-gray-500 py-2">
            No {importType === "invoices" ? "invoices" : "proforma invoices"}{" "}
            found
          </div>
        )}
        ...
        <Group justify="flex-end" className="mt-8">
          ...
          <Button
            ...
            disabled={!importBillId || !importType}
...
export default ImportModalUpdate;
